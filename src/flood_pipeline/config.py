...
class PipelineConfig:
    ...
    def gfm_mask_path(self) -> Path:
        """Whole-time max flood mask (reference/overlay; not a FLEXTH input now)."""
        return self.data_dir / GFM_MAX_NAME

    def gfm_sum_path(self) -> Path:
        """The optional temporal-sum raster (aggregation "sum" or "both")."""
        return self.data_dir / GFM_SUM_NAME

    def gfm_scene_path(self, stamp: str) -> Path:
        """Per-scene flood mask for one acquisition timestamp."""
        return self.data_dir / f"{GFM_SCENE_PREFIX}_{stamp}.tif"

    def gfm_scene_paths(self) -> list[Path]:
        """All per-scene flood masks, sorted by timestamp; excludes max/sum."""
        if not self.data_dir.exists():
            return []
        scenes = [
            p
            for p in self.data_dir.glob(GFM_SCENE_GLOB)
